...
const Facture = () => {
  ...
  return (
    <View style={{ padding: 40 }}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('create_facture', { client_id })}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 5 }}>
            <TouchableOpacity onPress={() => navigation.navigate('detail_facture', { facture_id: item.id })}>
              <Text style={{ flex: 1 }}>({item.num})</Text>
            </TouchableOpacity>
            <Text style={{ flex: 1 }}>{item.date}</Text>

            <TouchableOpacity onPress={() => navigation.navigate('edit_facture', { facture: item })}>
